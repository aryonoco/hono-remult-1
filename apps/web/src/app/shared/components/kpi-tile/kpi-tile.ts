...
@Component({
  ...
  template: `
    ...
    <ng-template #content>
      <span class="absolute inset-y-0 start-0 w-0.75" [class]="spineClass()" aria-hidden="true"></span>
      <span class="text-[0.65625rem] font-semibold uppercase tracking-[0.08em] text-on-surface-variant">{{
        label()
      }}</span>
      <span
