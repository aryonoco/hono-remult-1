...
- [ ] **DASH-3 (major): Every active incident reads as overdue ("−Nd")** — needs-attention sorts overdue-desc and seeded `nextReportDue` is in the past → all rows large-negative, diluting urgency. **NOT FIXED — absorbed into the FIXTURES campaign** (see Workstream below). A first attempt (FIXTURES workflow) FAILED: the implementer exceeded its context window; nothing committed; DB still FY2018-2026, all 13 active overdue. Redo via SMALLER context-safe sub-workflows. Fix: real-clock rolling active set so overdue is the exception. (`overview.ts`; seed data)
