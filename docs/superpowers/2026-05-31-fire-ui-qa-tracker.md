...
- [x] **FORM-1 (major): Material form fields don't bind `aria-invalid`** — errors render but field not marked invalid for SR. Fix: `[attr.aria-invalid]` (true-or-absent) per invalid field in `dynamic-form`. (`dynamic-form.ts`)
- [x] **FORM-2 (major): Dialog initial focus not managed** — opens focusing Cancel, not the primary action. Fix: `{ autoFocus: 'first-tabbable', restoreFocus: true }` (or order primary first). (`escalate-dialog.ts`, `confirm-dialog.ts`, `confirm-reason-dialog.ts`)
- [x] **FORM-3 (major): Dialog action buttons lack a clear focus ring (esp. dark)** — Fix: 2px primary `:focus-visible` ring via `mat.*-overrides()`/strong-focus, verified both themes. (`styles.scss`, `escalate-dialog.ts`)
- [x] **FORM-4 (minor): mat-select text/options not consistently themed to on-surface** — Fix: `mat.select-overrides()` for text colour (filters + dev-user-switcher). (`styles.scss`, `dev-user-switcher.ts`)
- [x] **FORM-5 (minor): Form "ready" precedes async option resolution** — brief incomplete form. Fix: gate ready on `!districtsResource.isLoading()` (and parent resource for sitrep). (`incident-form.ts`, `situation-report-form.ts`)
...
- [x] **FIRE-AREA-7 (major; user-requested 2026-06-01; FUTURE WORK — after FORMS+SCOPE+CSS, before the final sweep): Statewide fire-extent polygons on the `/overview` map.** The overview "Active incident map" plots only active fires; after the FIXTURES fix that is a small seasonal set (e.g. 4 in winter, mostly sub-hectare → no polygon), so the large historical extent polygons (7,712/21,129 fires carry one) are invisible on the landing page — they only show when drilling into a historical fire's detail. Add a statewide / recent-significant-fires extent layer to the overview map that draws the large `firePerimeterGeo` polygons so the showcase's perimeters are visible on first load. Scope-aware (statewide for elevated roles, district for district viewers — coordinate with SCOPE-1..5), bounded query (top-N by area / recent significant — NO wholesale load), token-only colour, AA both themes, reduced-motion framing. **Confirmed with the user: the existing 2D mapped-extent polygons are the desired representation (NOT literal 3D).** Browser-verify polygons render on `/overview` in light+dark as both an admin and a district viewer. (`features/overview/overview.{ts,html}`, `incident-map.ts`) _Note: FIRE-AREA-1..6 are done + verified; the perimeter feature itself works (Bairnsdale 59,783 ha detail map renders its `fire-polygon--safe` extent) — this item is purely about surfacing the existing polygons on the landing map._
...
- [x] **FU-1 (major): Modernise form-field rendering** — switch `<mat-form-field>` from default *fill* to M3 *outline* appearance in `dynamic-form.ts`; refine `mat.form-field-overrides()` so inputs/selects/textareas/datepickers read clean and professional in light+dark (outline, label, hint, error, focus colours via `--mat-sys-*`). (`dynamic-form.ts`, `styles.scss`)
- [x] **FU-2 (major): Section groups as cards, not faint fieldsets** — render each `FieldGroup` as a surface-container card (radius, subtle border/elevation, clear header + description) matching the detail panels, while KEEPING `<fieldset>/<legend>` semantics for a11y. (`dynamic-form.ts`)
- [x] **FU-3 (major): Responsive field grid** — fluid grid honouring `span` hints (full/half/third) that collapses to single-column ≤640px and 2-col mid; consistent gaps; no overflow at 390px. (`dynamic-form.ts`)
- [x] **FU-4 (major): Comfortable/Compact density toggle** — add a forms density toggle (mirroring the list pattern) driving theme-time Material density via scoped `mat.*-density`/overrides (NOT CSS padding); accessible toggle (aria-label, keyboard), persisted choice. **Use a WIDE gap (see DENSITY-1): compact noticeably denser, comfortable noticeably airier than the default.** (`dynamic-form.ts`/form pages, `styles.scss`)
- [x] **DENSITY-1 (major, user-requested): Global, persisted density preference + wider distinction** — density must be an APP-WIDE preference, not per-page: introduce a `DensityService` (mirroring `ThemeService` — a `signal<'comfortable'|'compact'>`, **default `compact` when unset**, persisted to `localStorage` key e.g. `fire-density`, reflected app-wide e.g. via `html[data-density]`). The incident list's local density signal is REFACTORED to consume it; the forms toggle (FU-4) consumes the same service; changing density on any surface updates it everywhere. **Widen the gap:** compact a step denser, comfortable a step airier than today (e.g. list rows ~56px comfortable / ~32px compact + matching field/control density). Theme-time density (`mat.*-overrides()`/`mat.*-density` scoped to `[data-density]`), not CSS padding; AA + ≥24px targets hold in compact. (`core/density.service.ts` NEW, `incident-list.{ts,html,css}`, `styles.scss`, form pages, `dynamic-form.ts`)
- [x] **FU-5 (major): Validation & required affordances + aria** — required marker, clear inline error styling, `[attr.aria-invalid]` per invalid field (folds in FORM-1), `aria-describedby` linking the `mat-error`, hint text. (`dynamic-form.ts`)
- [x] **FU-6 (major): Form page shells** — uplift the 3 page layouts: heading, reading-measure container, a clear (optionally sticky) action bar, spacing, logical tab order; Cancel/Save affordance + focus. (`incident-form.ts`, `situation-report-form.ts`, `final-report-form.ts`)
- [x] **FU-7 (minor): Control theming consistency** — mat-select/datepicker/textarea/slide-toggle all themed on-surface (folds in FORM-4), consistent focus rings, ≥24px targets. (`styles.scss`, `dynamic-form.ts`, `dev-user-switcher.ts`)
- [x] **FU-8 (minor): Dialog focus + rings** — folds in FORM-2 (`{autoFocus:'first-tabbable',restoreFocus:true}` + primary `cdkFocusInitial`) and FORM-3 (visible dialog button focus ring both themes). (`incident-detail.ts`, `unsaved-changes.ts`, dialog components, `styles.scss`)
- [x] **FU-9 (minor): Ready-gating** — folds in FORM-5: gate `pageState` ready on async option resolution (districts / parent resource). (`incident-form.ts`, `situation-report-form.ts`)
...
- [x] **SCOPE-1 (major): Reusable scope indicator** — a single, consistent scope affordance (e.g. a badge/subtitle
...
- [x] **SCOPE-2 (major): Overview scope labelling** — the `/overview` H1 / "Active operations" band / KPI captions /
...
- [x] **SCOPE-3 (major): Incident list scope labelling** — the list a district viewer sees is already filtered to their
...
- [x] **SCOPE-4 (minor): Detail + forms scope context** — confirm the incident detail and the 3 form pages make their
...
- [x] **SCOPE-5 (minor): A11y + correctness of scope** — the scope indicator must be a real text affordance (not colour
...
- **FIXTURES (FY2029 + real-clock rolling active set) — DONE, browser-verified** (commits `f5d66aa` seed, `b42e551` eslint out-tsc ignore). Dropped the hardcoded `ANCHOR`; the seed now generates a complete seasonally-weighted FY2018-FY2029 record (analogue years FY2027-29 = FY2018/19/20 ±15% deterministic jitter, clamped to a fixed `dataHorizon`) plus a rolling-active overlay anchored to an injectable real-clock `now`: seasonally-scaled size (peak 60 / winter floor 4 so every interim status shows), recent ignitions, and only a `round(count*overdueShare)` minority overdue. Active fires now always file ≥1 sitrep so their denormalised status is backed by a real report. Re-seeded (21,129 fires; active=4 upcoming=3 overdue=1). **DASH-3 browser-verified** at `/overview` as Sarah Admin (statewide): cadence chips read "1d overdue / in 2h 59m / in 2d / in 3d"; map plots all 4 active fires (12 CARTO tiles); the SSE live machinery is healthy (`/api/stream`, both `/stream/subscribe`, `_liveQueryKeepAlive` all 200; honest "Live"; **0 console errors** across a 14s session past the 10s idle-timeout — the one initial-connect EventSource error on a cold dev server self-recovers). `check:ci` + api (40) + shared-domain green; zero suppressions (one pre-existing `noBitwiseOperators` ignore on the PRNG seed mix, matching `prng.ts`).
- **FIXTURES + FORMS first attempts — FAILED (context-window), reverted.** Both the FIXTURES and FORMS-CORE single-implementer workflows exceeded the model context window (seed + forms-engine codebases are large) and committed NOTHING; FORMS-CORE's partial uncommitted edits were discarded. DASH-3 remains OPEN; the dataset is still FY2018-2026 with all active fires overdue. **Lesson: split FIXTURES + FORMS into smaller, tightly-scoped (2-4 files each) sub-workflows and run strictly ONE workflow at a time** (concurrent workflows on the shared git index also contributed). Briefs remain valid: `.superpowers/fixtures-extension-brief.md`, `.superpowers/forms-core-brief.md`, `.superpowers/forms-pages-brief.md` (decompose them further at dispatch).
...
- **FIRE-AREA-7 — DONE, browser-verified as both roles** (commit `aad6f28`). The `/overview` map now overlays the
  largest recent fires that carry a perimeter so the dataset's mapped extents are visible on the landing page (the
  live active set is small + sub-hectare in a quiet season). Bounded `find`: top fires by area (floor 100 ha),
  `reportedAt <= now` (future seasons stay hidden), over-fetched to 36 then trimmed to the 12 carrying a perimeter,
  excluding any already in the active set (no double-draw). **Scope-aware for free** via the FireIncident
  apiPrefilter — admin sees statewide extents, a district viewer sees their district's. Heading renamed "Incident
  map" + an honest caption ("Active incidents, plus the N largest recent fire extents …"); the active map-overflow
  note stays about the active set alone. **Fixed a latent Leaflet crash this surfaced:** `initMap` added vector
  layers before framing the view, so a freshly-created map gave its SVG renderer no pixel bounds and
  `_clipPoints` threw on an undefined bound — only reliably hit by the dozen large statewide polygons. Frame first
  (framing is derived from point data, not layers), then draw; the detail map is unchanged. Browser-verified via a
  playwright-core probe (`/tmp/pwcap/verify-firearea7.mjs`): admin `/overview` = 13 polygons + 2 circles + 32
  markers, Otway viewer = 12 Otway extents, light + dark, **0 console errors**; Bairnsdale detail still draws its
  single extent. New overview spec test (seeded perimeter fire → significantCount/caption); `check:ci` + 293 web
  tests + AA guard green; zero suppressions.
